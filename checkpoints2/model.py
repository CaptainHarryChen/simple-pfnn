...
class SimplePFNN:
    def __init__(self, input_dim, output_dim, hidden_dim=512):
        ...
        self.ELU1 = ELU()
        self.linear2 = Linear(hidden_dim, hidden_dim * 2)
        self.ELU2 = ELU()
        self.linear3 = Linear(hidden_dim * 2, hidden_dim)
        self.ELU3 = ELU()
        self.linear4 = Linear(hidden_dim, output_dim)

    def __call__(self, x, phase):
        x = self.ELU1(self.linear1(x, phase))
        x = self.ELU2(self.linear2(x, phase))
        x = self.ELU3(self.linear3(x, phase))
        x = self.linear4(x, phase)
        return x
    
    def backward(self, grad_y):
        grad_x = self.linear4.backward(grad_y)
        grad_x = self.ELU3.backward(grad_x)
        grad_x = self.linear3.backward(grad_x)
    ...
    def save(self, filename):
        np.savez(filename, 
            ...
            linear4_weight=self.linear4.weight,
            linear4_bais=self.linear4.bais
        )
    ...
    def load(self, filename):
        with np.load(filename) as f:
            ...
            self.linear4.weight = f["linear4_weight"]
            self.linear4.bais = f["linear4_bais"]
