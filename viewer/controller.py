...
class KeyAndPad():
    ...
    def update_gamepad(self, task):
        ...
        left_trigger = self.device.findAxis(InputDevice.Axis.left_trigger).value
        right_trigger = self.device.findAxis(InputDevice.Axis.right_trigger).value
        self.viewer.cameractrl.updateGamepad((left_trigger * 2 - 1) * 0.3, (right_trigger * 2 - 1) * 0.3, task)
        return task.cont
